...

{
	printf("\n\n\tPLEASE FOLLOW THE BELOW INSTRUCTIONS FOR PROPER FUNCTIONING OF THE PROGRAM"
			...
			"\n\n\t>Query ID entered should be unique and should be an integer value\n"
	...
	if(n==0) { printf("\n No queries to solve\n"); }
		...
		for(i=0; i<n; i++){
			...
			if(priority==1)
			{
				...
				if(fac[fc].bt<=0)
				{
					printf("\n\n\tResolving time cannot be less than or equal to zero Press enter");
					getch();
				processDetails();
				}
...

{
	...
	
	{
		...
		else if((m[i].cmpt+1000)>=1060 && (m[i].cmpt+1000)<1160)
		{
		printf("\n\t%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t\t%d",
		m[i].priid,m[i].qid, (m[i].at+1000), m[i].bt, (m[i].cmpt+1000+100-60), (m[i].cmpt-m[i].at), ((m[i].cmpt-m[i].at)- m[i].bt));
			
		}
		
		
		else if((m[i].cmpt+1000)>=1160)
		{
			printf("\n\t%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t\t%d",
		m[i].priid,m[i].qid, (m[i].at+1000), m[i].bt, (((m[i].cmpt)/(m[i].cmpt))+1200-1), (1200-(m[i].at+1000)-40), ((m[i].cmpt-m[i].at)- m[i].bt));
		count++;
		}
		...
		if(m[i].cmpt+1000>=1100)
		{
		
		total= (m[i].cmpt+1000-1100+60);
